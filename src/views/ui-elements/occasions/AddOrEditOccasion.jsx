...
import { useAddOccasion, useUpdateOccasion } from '../../../hooks/occasions/useOccasionsMutation';
import { useNavigate } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import { getOccasionById } from '../../../api/occasions';

const AddOrEditOccasions = () => {
  ...
  const { data: detail, isLoading: loadingDetail } = useQuery({
    ...
    queryFn: () => getOccasionById(id), // should return the doc object
    enabled: isEdit && !!id,
    select: (doc) => doc || {}
  });
  ...
  const { mutateAsync: addMutation, isPending: addPending } = useAddOccasion();
  const { mutateAsync: updateMutation, isPending: updatePending } = useUpdateOccasion();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        ...
        // reset for add
        revokeIfBlob(form.imagePreview);
        setForm({ name: '', imageFile: null, imagePreview: '' });
      }
      // IMPORTANT
  ...
  const title = isEdit ? 'Edit Occasion' : 'Add Occasion';
  const btnLabel = isEdit ? 'Save Changes' : 'Add Occasion';
...
export default AddOrEditOccasions;
