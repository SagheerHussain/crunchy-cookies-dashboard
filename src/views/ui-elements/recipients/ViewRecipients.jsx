  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        ...
        >
          <Tooltip title="Edit">
            <IconButton size="small" onClick={() => navigate(`/recipients/edit/${params.row.id ?? params.row._id}`)}>
              <EditIcon fontSize="small" sx={{ color: '#fff' }} />
            </IconButton>
          </Tooltip>
          <Tooltip title="Delete">
            <IconButton size="small" color="error" onClick={() => openConfirm(params.row)} disabled={deleteRecipientPending}>
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      >
        ...
        <Button isLink to="/recipients/add" isStartIcon startIcon={<IoBag />} variant="contained" color="primary">
          Add Recipient
        </Button>
      </div>
      ...
      <Dialog open={confirm.open} onClose={deleteRecipientPending ? undefined : closeConfirm}>
        ...
        <DialogActions>
          ...
          <MuiButton onClick={handleConfirmDelete} color="error" variant="contained" disabled={deleteRecipientPending}>
